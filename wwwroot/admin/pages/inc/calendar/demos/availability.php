...
<?php
$query2 = 'SELECT * FROM snapbid_room_availability LEFT JOIN snapbid_rooms ON snapbid_room_availability.availability_room_id = snapbid_rooms.room_id JOIN snapbid_hotels ON snapbid_rooms.hotel_id = snapbid_hotels.id WHERE availability_room_id = '.$room_id.'';
	...
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			...
			<?php foreach($rows2 as $row2) : ?>			
				{	
					id: "<?php echo $row2['availability_id']; ?>",
					title: "<?php echo $row2['availability_amount']; ?>",
					start: "<?php echo $row2['availability_date_from']; ?>",
					end: "<?php echo $row2['availability_date_to']; ?>",
					<?php if($row2['availability_recurring'] !== "") : ?>
						dow: [ <?php echo $row2['availability_recurring']; ?> ],
					<?php endif; ?>
				},
			<?php endforeach ?>
			]
