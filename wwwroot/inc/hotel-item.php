<?php foreach($rows as $row) : ?> 
   <div price="<?php echo $row['price']; ?>" class="col-xs-12 hotel-item star<?php echo $row['star_rating']; ?> <?php $facilities = str_replace(" ", "-", $row['features']);  echo str_replace(",-", " ", $facilities); ?> <?php if(isset($row['reserve_price'])) : ?>Bid<?php endif; ?>">

    <div class="col-xs-4 hotel-image">
        <a href="hotel.php?hotel_id=<?php echo $row['id']; ?>">
			<div style="background-image:url(<?php echo $row['main_image']; ?>);background-size:cover;background-position: center;" class="inner"></div>
		</a>
    </div>
